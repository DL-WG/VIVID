...
#######################################################################################
# conventional DA with POD

u_pod = np.load('data/u_pod_train_final.npy')[:,:100]
u_b = np.load('data/xb_DA_4000.npy')

def obs_operator(x):
  x_r = np.dot(u_pod,x).reshape(-1,1)
  x_m = obs_field (x_r,H).reshape(-1,1)
  return np.dot(obs_matrix,x_m)
...
case.set( 'Background',          Vector=ub_compress.ravel())
case.set( 'BackgroundError',     ScalarSparseMatrix=1000.0 )
#case.set( 'BackgroundError',     Matrix= 1000*B )
...
POD_initial_predict = x_a.reshape(-1,1)
...
print('analysis error',np.linalg.norm(X_a - u)/np.linalg.norm(u))
print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
from skimage import data, img_as_float
from skimage.metrics import structural_similarity as ssim
print('background ssim',ssim(u_b.reshape(50,50),u))
print('analysis ssim',ssim(X_a,u))
########################################################################################

im = plt.imshow(np.abs(X_a-u),vmin=0,vmax=0.02,cmap='jet')
plt.colorbar(im)
plt.axis('off')

im = plt.imshow(X_a,vmin=-0.03,vmax=0.03)
plt.colorbar(im)
plt.axis('off')
...
model =  keras.models.load_model('model/SW_VC_u_random3_POD_final.h5')
...
CNN_inverse = model.predict(Z_nearest.T.reshape(1,50,50))

y_combined = np.array(list(CNN_inverse.ravel()) + list(y.ravel()))
def obs_operator(x):
  x_r = np.dot(u_pod,x).reshape(-1,1)
  x_m = obs_field (x_r,H).reshape(-1,1)
...
R_diag = list(300*np.ones(100))+list(1*np.ones(100))
...
X_a = np.dot(u_pod,x_a.reshape(-1,1)).reshape(50,50)
im = plt.imshow(np.dot(u_pod,x_a.reshape(-1,1)).reshape(50,50),vmin=-0.03,vmax=0.03)
plt.colorbar(im)
plt.axis('off')

im = plt.imshow(np.abs((np.dot(u_pod,CNN_inverse.reshape(-1,1)).reshape(50,50)-u)),vmin=0,vmax=0.02,cmap='jet')
plt.colorbar(im)
plt.axis('off')


print('background error',np.linalg.norm(u_b.reshape(50,50) - u)/np.linalg.norm(u))
print('inverse error',np.linalg.norm(np.dot(u_pod,CNN_inverse.reshape(-1,1)).reshape(50,50) - u)/np.linalg.norm(u))
print('analysis error',np.linalg.norm(X_a - u)/np.linalg.norm(u))
print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
from skimage import data, img_as_float
from skimage.metrics import structural_similarity as ssim
